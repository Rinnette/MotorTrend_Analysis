...
Linear regression was used with MPG as the outcome and *am* the predictor.
...
Checking to see how mpg varies with the other variables in the dataset.
...
*cyl*, *wt*, *disp*, *hp*, *drat* and *vs* show the highest correlation coefficients.


Pairs plots were done for these variables.

```{r fPlot}
        featurePlot(x=m[, c("cyl", "wt", "disp", "hp", "drat", "vs")], y=m$mpg, plot="pairs")
        #featurePlot(x=m[, c("qsec", "vs", "gear", "carb")], y=m$mpg, plot="pairs")
```

The correlation coefficients and scatter plots show strong relationships between *mpg* and *cyl*, *wt*, *disp*, *hp*, *drat* and *vs* (listed in order with *cyl* being the strongest).

The following models were built with the following as predictors:

* *am* + *cyl*
* *am* + *cyl* + *wt*
* *am* + *cyl* + *wt* + *disp*
* *am* + *cyl* + *wt* + *disp* + *hp*
* *am* + *cyl* + *wt* + *disp* + *hp* + *drat*
...
```{r}
        ...
        ar2 <- c(summary(m2)$adj.r.squared, summary(m3)$adj.r.squared, summary(m4)$adj.r.squared, summary(m5)$adj.r.squared, summary(m6)$adj.r.squared, summary(m7)$adj.r.squared)
        
        ar2
```

The figures show the adjusted R-squared values after each predictor was added.  After adding *drat* the value decreases. Model m5 therefore shows the highest adjusted r-squared of about 83%.
...
```{r plotResid}
        plot(m5, which = 1)
```
Residuals are almost all on the zero line. Also shows outliers at row numbers 20,  29, and 31.
...
All the dfbetas and hat values are within the thresholds so therefore the outliers are not very influential over the prediction model.


```{r}
        ## checking normal Q-Q plot
              plot(m5, which = 2)
```

Since the points are almost all along the diagonal, this shows that the residuals are approximately normally distributed with mean 0 and standard deviation of 1.
