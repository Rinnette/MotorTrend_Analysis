...
The structure of the dataset and the number of vehicles of each transmission type were checked, as well as the average mpg for each type of transmission was calculated.  Note that in the *am* variable, 0 represents automatic transmission and 1 represents manual transmission.  
...
The last set of values shows that the manual cars in the dataset have on average higher MPG than the automatics cars. 

A boxplot of the data was done on mpg and transmission type and this also showed that generally manual cars get higher MPG than automatic cars.  This figure can be found in the Appendix.
...
Fit initial model with *am* as the predictor:
\scriptsize
```{r fitInitModel}
        # Fit the initial model with am only
                m$am = factor(m$am)
                m1 <- lm(mpg ~ am, m)
